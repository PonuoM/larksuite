...
export default function FormattedReport({ content }: { content: string }) {
 return <div className="formatted-report">{formatReport(content).map((block,i)=> {
  ...
  if(block.kind==='list')return <ul key={i}>{block.lines.map((line,j)=> { const trimmed=line.trimStart(); const depth=Math.min(3,Math.floor((line.length-trimmed.length)/2)); const text=trimmed.replace(/^[-*+]\s+|^\d+[.)]\s+/,''); const check=text.match(/^\[([ xX])\]\s*(.*)/); const className=[depth?`depth-${depth}`:'',check?'check':''].filter(Boolean).join(' ')||undefined; return <li key={j} className={className}>{check ? <><span aria-label={check[1]===' '?'ยังไม่เสร็จ':'เสร็จแล้ว'}>{check[1]===' '?'☐':'☑'}</span> {inline(check[2])}</> : inline(text)}</li>; })}</ul>;
