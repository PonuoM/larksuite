...
type Key = 'id' | 'title' | 'project' | 'kind' | 'size' | 'status' | 'assignee' | 'progress' | 'go_live' | 'updated';
const SIZE_RANK: Record<string, number> = { S: 1, M: 2, L: 3 };

export default function TaskTable({ tasks, projects, canApprove, moving, onOpen, onApprove }: { tasks: Task[]; projects: Project[]; canApprove: boolean; moving: number[]; onOpen: (t: Task) => void; onApprove: (t: Task) => void }) {
  ...
  return <div className="table-wrap">
    <table className="task-table" aria-label="ตารางงาน">
      <thead><tr>
        ...
        {head('status', 'สถานะ')}{head('assignee', 'ผู้รับผิดชอบ')}{head('progress', 'งานย่อย')}{head('go_live', 'กำหนดเริ่มใช้')}{head('updated', 'แก้ไขล่าสุด')}
      </tr></thead>
      <tbody>{rows.map((t) => {
