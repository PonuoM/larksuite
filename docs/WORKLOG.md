...
## 2026-09-19 — overview page as a card dashboard
Owner sent a dark dashboard reference and asked for the same card/chart style in white. src/ProjectViews.tsx rewritten (project-brief.css now holds the dash-* styles); no chart library, hand-drawn SVG/CSS. Cards: hero (งานยังไม่จบ, เปิดใช้ในสัปดาห์ที่เลือก with change vs the previous week, งานย่อยเสร็จ %, รออนุมัติ/ติดขัด/เลยกำหนด chips, status bar + legend) · ความเคลื่อนไหว heatmap, 12 weeks × 7 days, counting sub-tasks ticked (done_at), releases and each task's latest edit (only the latest edit is stored; the caption says so) · กำหนดเริ่มใช้ vs เปิดใช้จริง, 8 weeks around the selected week, clicking a week selects it · ประเภทงาน bars (open / released) · ต้องดูตอนนี้ tabs + rows · สถานะงาน donut · แต่ละโปรเจกต์ถึงไหน (collapsible, status bar, % released, open board) · ตารางสัปดาห์ on the right: day chips, releases and planned go-live dates in the week (past planned = เลยกำหนด). Week picker gained ‹ › and สัปดาห์นี้; download unchanged.
Checks: tsc, build, presentation/report-format/task-search tests. Browser (Chrome via Playwright) at 1600, 1200 and 375 px with synthetic tasks injected by request interception (local DB has no tasks; nothing written): no horizontal overflow at any width. Not checked against production data yet.
Deployed bbb627e (owner said go): backup workboard-2026-09-19.sql.gz, git archive, build + up -d app only (no migration). After: HTTPS 200, /api/v1/session ok, live bundle contains the dashboard.
- 2026-09-19: overview dashboard motion — cards rise in with a stagger, numbers count up/glide, status bars/kind bars/donut grow from zero and glide on filter or week change, trend lines reveal left-to-right when the data changes, rows and schedule items cascade; all off under prefers-reduced-motion. Checked in Chrome (frames at 250 ms/350 ms and settled, filter change, 1600 and 375 px, no console errors). Deployed.
- 2026-09-19: approver (view-only) task panel shows scope / criteria / blocked reason / evidence through FormattedReport instead of raw Markdown (## and - were visible), each in its own card. Checked in Chrome with a mocked approver session. Deployed.
- 2026-09-19: edit a member's projects after the link was sent. Owner: new projects keep being added but old links could not get them. Access is stored per member (memberships), not per link, so a new endpoint POST /access/{id}/projects {role: viewer|editor, project_ids} replaces the member's memberships in one transaction (admins, revoked members and empty/unknown selections refused). Settings → การเข้าถึง gets a "แก้ไขโปรเจกต์" button per non-admin member: project checkboxes + "ทุกโปรเจกต์ที่มีตอนนี้" + ดู/แก้ไข. Applies to every existing link and open session at once (roleFor reads memberships per request); an open page sees added projects after a reload. Mixed per-project roles are not offered — saving sets one role for all the member's projects, same as when the link was created. Checks: tsc, build, tests/api.mjs (+ grow/shrink through an already-open session, role change, invalid input, admin target 422, viewer 403); Chrome at 1400 and 390 px: open editor, add a project, header updates, restore, no horizontal overflow.
- 2026-09-19: slow pages for a view-only user (calendar 3–4 s). Not the database: indexes cover the task/meeting queries and each API call takes ~5 ms on the server (~50 ms from outside = network round trip). Cause was Apache prefork behind Caddy: KeepAlive On held a worker 5 s on each of Caddy's idle upstream connections, so the calendar's 18 parallel requests (6 projects × 3 months) needed 18 workers while only 10 idle existed, and prefork spawns 1, 2, 4, 8 per second. Measured on production with an HTTP/2 client (as a browser): cold burst slowest 3,476 ms, worker count stepping 10→11→13→17→25 once per second, request completions in matching 1 s steps. Fix in deploy/Dockerfile (our container only, Caddy untouched): KeepAlive Off, StartServers/MinSpareServers 10, MaxSpareServers 20 (~7 MB per worker). Deployed 166c617 (apachectl -t in a throwaway container first). After: cold calendar burst 117 ms, repeats ~105 ms; tasks for 6 projects ~105 ms with gzip; Chrome on production, board → calendar 109–170 ms. Not done (not needed now): one batched meetings request instead of projects × months.
